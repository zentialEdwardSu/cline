...
import { ApiHandlerOptions, SFDeepSeekModelId, ModelInfo, sFDeepSeekDefaultModelId, sFDeepSeekModels } from "../../shared/api"
import { convertToOpenAiMessages } from "../transform/openai-format"
import { ApiStream } from "../transform/stream"
import { convertToR1Format } from "../transform/r1-format"

export class SFDeepSeekHandler implements ApiHandler {
    ...
    async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
        ...
        const stream = await this.client.chat.completions.create({
            ...
            ...(model.id === "Pro/deepseek-ai/DeepSeek-R1" ? {} : { temperature: 0 }),
        })
    ...
    getModel(): { id: SFDeepSeekModelId; info: ModelInfo } {
        const modelId = this.options.apiModelId
        if (modelId && modelId in sFDeepSeekModels) {
            const id = modelId as SFDeepSeekModelId
            return { id, info: sFDeepSeekModels[id] }
        }
        return {
            id: sFDeepSeekDefaultModelId,
            info: sFDeepSeekModels[sFDeepSeekDefaultModelId],
        }
    }
}
